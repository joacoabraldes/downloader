"""Calibra X-13 para producción de leche contra la columna `desest` de leche.xlsx.

Barre mode x td x seasonalma, corre x13as sobre la serie observada (col `valor`, en litros) y
compara el d11 resultante contra la referencia (col `desest`).

La variante ganadora reproduce la referencia con error ~0: mode=add, td1coef, s3x5. Los
valores son litros (~9 dígitos): el núcleo (etl.core.seasonal) envuelve las líneas del .spc
por ancho de caracteres para que X-13 no parta un número (si no, falla el ajuste).

Uso: python scripts/calibrar_leche.py
"""
import os
import subprocess
import sys
...
XLSX = os.path.join(os.path.dirname(__file__), os.pardir,
                    "etl", "datasets", "leche", "data", "leche.xlsx")


def load():
    ws = openpyxl.load_workbook(XLSX, data_only=True)["Mensual"]
    dates, obs, ref = [], [], {}
...
def run(x13, dates, obs, ref, mode, td, sma):
    wd = tempfile.mkdtemp(prefix="cal_leche_")
...
def main():
    ...
    print(f"\n{'mode':5} {'td':8} {'sma':5} {'arima':16} {'meanpct%':>10} {'maxpct%':>10}")
    for r in results:
        print(f"{r['mode']:5} {r['td']:8} {r['sma']:5} {str(r['arima']):16} "
              f"{r['meanpct']:10.6f} {r['maxpct']:10.6f}")
