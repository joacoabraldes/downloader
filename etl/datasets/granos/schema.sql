...
create table if not exists etl_molienda_granos (
...
alter table etl_molienda_granos add column if not exists parametros jsonb;

-- Búsqueda del último snapshot de un (serie, date, estado).
create index if not exists etl_molienda_granos_serie_date_estado_idx
    on etl_molienda_granos (serie, date, estado, ingested_at desc);

-- Una sola fila desestacionalizada por (serie, mes) (UPSERT desde el núcleo X-13).
create unique index if not exists etl_molienda_granos_desest_uq
    on etl_molienda_granos (serie, date)
...
create or replace view etl_molienda_granos_actual as
select distinct on (serie, date)
    serie, date, valor, estado, fuente, ingested_at
from etl_molienda_granos
...
create or replace view etl_molienda_granos_desest as
select distinct on (serie, date)
