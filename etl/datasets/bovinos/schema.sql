...
create table if not exists etl_bovinos (
...
alter table etl_bovinos add column if not exists parametros jsonb;

-- Búsqueda del último snapshot de un (serie, date, estado).
create index if not exists etl_bovinos_serie_date_estado_idx
  on etl_bovinos (serie, date, estado, ingested_at desc);

-- Una sola fila desestacionalizada por (serie, mes) (UPSERT desde el núcleo X-13).
create unique index if not exists etl_bovinos_desest_uq
  on etl_bovinos (serie, date)
...
create or replace view etl_bovinos_actual as
...
create or replace view etl_bovinos_desest as
select distinct on (serie, date)
