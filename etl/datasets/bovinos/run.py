...
snapshotea la serie con estado='definitivo'. El xls re-publica la serie 2019→, así que
insert_if_changed absorbe las revisiones de MAGyP y se pone al día solo. Al final, sólo si
hubo datos nuevos o actualizados, desestacionaliza (X-13).
...
def main(argv=None) -> None:
    ...
    try:
        ...
        if not res or not res[0]:
            rep.info("no se pudo descubrir/parsear el xls de faena bovina")
            rep.summary()
            return
        data, url = res
        ...
        for fecha in sorted(data):
            ...
            rep.tally(status)  # ~86 meses: se cuentan, no se imprime línea por mes
        rep.summary()
