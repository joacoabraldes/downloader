...
alter table aves add column if not exists parametros jsonb;

-- Búsqueda del último snapshot de un (serie, date, estado).
create index if not exists aves_serie_date_estado_idx
  on aves (serie, date, estado, ingested_at desc);

-- Una sola fila desestacionalizada por (serie, mes) (UPSERT desde el núcleo X-13).
create unique index if not exists aves_desest_uq
  on aves (serie, date)
...
create or replace view aves_actual as
...
create or replace view aves_desest as
select distinct on (serie, date)
