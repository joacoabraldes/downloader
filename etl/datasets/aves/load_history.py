"""Carga histórica (one-off) de faena avícola desde el Excel de referencia (Aves.xlsx).

El Excel tiene una hoja con columnas `date | valor | desest`: `valor` es la faena mensual
observada (miles de cabezas, desde 1981-01) y `desest` es la referencia de X-13 (no se
ingesta; sirve para calibrar). Se carga `valor` con estado=NULL (histórico). Idempotente:
usa insert_if_changed. El indicador mensual de MAGyP (run.py) re-publica los últimos años
como 'definitivo', que en la vista _actual tienen prioridad sobre este histórico.
"""
...
DEFAULT_XLSX = Path(__file__).parent / "data" / "Aves.xlsx"
SHEET = "Hoja1"
...
def main(argv=None) -> None:
    ap = argparse.ArgumentParser(prog="etl aves load-history",
                                 description="Carga histórica del Excel de aves (one-off).")
    ...
    rep = report.Report("aves", "load-history")
